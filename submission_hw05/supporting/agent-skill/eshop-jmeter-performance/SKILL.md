...
1. Inspect the repository and endpoint implementation before generating a plan. For the bundled workflow, read [references/scenario-b.md](references/scenario-b.md). For another endpoint group, read [references/extension-guide.md](references/extension-guide.md) and create an endpoint contract from `assets/endpoint-contract-template.json`.
2. Validate CSV inputs, correlations, assertions, filename, and XML with `scripts/validate_jmx.py`. Pass `--contract <path>` for a non-default endpoint group.
...
For another endpoint group, replace only the endpoint contract, CSV schema, correlations, assertions, and business sequence. Reuse the preflight, approval, CLI execution, immutable evidence, JTL analysis, manifest, and human-review workflow unchanged.
