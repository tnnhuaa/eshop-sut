...
Automated runs begin as `RUN_UNVERIFIED`. The pipeline may pass or fail a CI gate, but it cannot label evidence `HUMAN_VERIFIED`. A student or reviewer must inspect and explicitly accept it. One failed run creates a warning or failed job, not a GitHub defect. A defect requires repeatable evidence, exclusion of test/data/environment causes, and explicit human confirmation.
