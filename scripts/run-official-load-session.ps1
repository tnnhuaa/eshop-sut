...
$evidenceDir = Join-Path $repoRoot "docs\evidence\load\$RunId"
...
Write-Host "OFFICIAL LOAD APPROVAL: $RunId"
Write-Host "RESET AND SEED: $ResetId"
& $node $seedScript "--users=10" "--max-uses=100" "--reset-id=$ResetId"
if ($LASTEXITCODE -ne 0) { throw "Official Load seed failed" }
...
if ($preflightRows.Count -ne 9 -or $failedPreflightRows.Count -ne 0 -or $preflightLabels.Count -ne 9) {
    throw "PREFLIGHT FAILED: expected 9 unique successful samples; official Load was not started"
}
...
$monitorJob = Start-Job -ScriptBlock {
    ...
    Get-Counter -Counter $counterPaths -SampleInterval 1 -MaxSamples 450 | ForEach-Object {
...
try {
    Write-Host "OFFICIAL LOAD START: L=10, ramp=120s, hold=300s"
    & $runner `
        -Plan $plan `
        -RunId $RunId `
        -Threads $CalibratedL `
...
Write-Host "OFFICIAL LOAD FINISHED AS RUN_UNVERIFIED"
